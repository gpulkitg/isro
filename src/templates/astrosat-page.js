...
export const query = graphql`
  query($slug: String!) {
    astrosatPagesYaml(slug: {eq: $slug}) {
      cover {
        title
        image {
          name
...
export default function AstrosatPage({ data }) {

  const {
    cover,
    title,
    sections,
  } = data.astrosatPagesYaml
              ...
              <Row className="d-flex align-items-end mb-2">
                { section.gifs.map((item, ind) => (
                  <Col key={item.gif.name} md>
                    <img src={item.gif.publicURL} alt={item.gif.name} />
                    <p className="text-secondary text-center">{item.caption}</p>
                  </Col>
                ))}
              </Row>
            } */}
